...
# ╔═╡ 5525302b-c0c0-4b19-8836-4032e34b2028
begin
...
# ╔═╡ b5fa073c-a875-11ec-3cbf-212c043749e3
md"## Load packages"

# ╔═╡ c2afdc47-1eeb-4d11-9b35-f07aecbed83b
md"## Set parameter values"

# ╔═╡ 9f72787f-c295-4bcb-ab68-f3703e5c1e2b
begin
	const T0 = 352.6634 # K
	const V = 0.1 # m^3
	const R = 8.314 # kJ.kmol^-1.K^-1
	const ΔH = -4.78E4 # kJ.kmol^-1
	const k0 = 72E9 # min^-1
	const E = 8.314E4 # kJ.kmol^-1
	const cp = 0.239 # kJ.kg^-1.K^-1
	const ρ = 1000 # kg.m^-3
	const f = 200E-3 # m^3.min^-1
	const CA0 = 0.7867 # kmol.m^-3
	const Q = 0 # kJ/min
end

# ╔═╡ fca011c7-8759-4dee-9845-be6baa2ade18
md"## Functions"

# ╔═╡ af7825ad-9774-4042-9ecd-42f991ec32fa
md"#### Open-loop CSTR model"

# ╔═╡ 49c9f1aa-24d9-4201-9b4b-4d8bf76e576b
function CSTR_OL!(dx,x,p,t)
    """
    Open loop CSTR simulation
    inputs:
        dx = derivative vector, [dCA/dt dT/dt]
        x = variable vector, [CA T]
        p = handled by DifferentialEquations.jl
        t = time; handled by DifferentialEquations.jl
    """
    dx[1] = (f/V)*CA0 - (f/V)*x[1] - k0*exp(-(E/(R*x[2])))*x[1]
    dx[2] = (f/V)*T0 - (f/V)*x[2] + (-ΔH)/(ρ*cp)*k0*exp(-(E/(R*x[2])))*x[1] + Q/(ρ*cp*V)
end

# ╔═╡ 5baff2a5-cf36-4744-b47d-a31adddb5a53
md"#### Open-loop CSTR simulation & plot"

# ╔═╡ 29047e54-81da-4ffd-b7c1-e8fd5ead5993
function OL_CSTR_sim(CA0=0.5734,T0=395.3258)
    x0=[CA0;T0] # initial values; uses defaults if not specified
    tspan = (0.0, 10.0)
    model = ODEProblem(CSTR_OL!, x0, tspan) # DifferentialEquations.jl model statement
    sol = solve(model, DP5()) # DP5() is the chosen ODE solver

    p1 = plot(sol,vars=(1),ylabel="CA",legend=false,ylim=(0.0, 1.0)) # plot CA var sol
    p2 = plot(sol,vars=(2),ylabel="T",legend=false,ylim=(300, 750)) # plot T var sol
    plt = plot(p1, p2, layout = (2,1)) # subplot with both vars
    return plt, sol
end

# ╔═╡ 8427bdb6-6176-4ccb-922b-aabcbafdd150
md"## Run open-loop simulation"

# ╔═╡ 362d6227-e869-4746-8d5e-7b005f4b1da6
md"Set a value for CA0"

# ╔═╡ 3decf840-6342-4d32-ae97-b4e7d3dd6ee6
md"0.2 - 1.0 kmol/m³"

# ╔═╡ 1ce08004-145a-4742-b031-6e456bb0ae90
@bind CA0_OL Slider(0.2:0.01:1.0;show_value=true)

# ╔═╡ 4e7177fb-2ef7-4301-b315-54ea32e55e59
md"Set a value for T0"

# ╔═╡ 9b6722f7-07b8-4bfc-83d6-b6764c60706f
md"340 - 520 K"

# ╔═╡ e273168e-93fd-4c4c-a109-296bf0ab5345
@bind T0_OL Slider(340:1:520;show_value=true)

# ╔═╡ aed4058e-5c4b-415e-9caa-ed75f32acf8b
begin
	plt, sol = OL_CSTR_sim(CA0_OL,T0_OL);
	plt
end
